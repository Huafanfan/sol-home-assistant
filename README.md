...
| macOS 麦克风与扬声器 | 已实现，部分真实硬件验收已通过 | Swift/AVFoundation 宿主、同机二进制协议、真实麦克风→云端→扬声器中文单轮、启动前零调用、手动打断、用量与无落盘复核均已通过；设备变化与拒绝权限由用户暂缓，故仍为 `implemented`。 |
| 唤醒词与自动 VAD | 规格已接受；离线基础已实现 | SOL1 v2 显式能力帧、可注入的本地唤醒/VAD 状态机和 Gateway `speech_started` 门控已有离线测试；尚未接入真实模型、麦克风或云端。 |
| AEC、降噪与免手连续对话 | 计划中 | 将在本地唤醒/VAD 之后以独立规格和房间实测决定，不以 VOICE-005 第一片的实现冒充已解决。 |
| 长期记忆 | 计划中 | 保留策略、数据模型、删除语义和审计边界已经先行定义。 |
| 智能家居写操作、联网搜索 | 不在 MVP 范围内 | 这些能力必须单独定义权限、确认、成本和审计规则。 |

一句话概括：文本推理与腾讯云实时语音已经完成真实验收，VOICE-004 已完成 Mac mini 的真实中文单轮、启动前零调用、手动打断及用量/无落盘复核；VOICE-005 的本地唤醒与自动 VAD 第一片已完成规格、ADR 与离线协议/状态机基础，但尚无真实模型、监听或云端验收。
...
1. 保留 VOICE-004 的真实设备变化与拒绝权限验收为暂缓项；实际遇到对应问题时补测和修复。
2. 继续接入 VOICE-005 第一片：将已完成的 v2 协议、离线状态机和 Gateway 门控接入前台本地监听，再按授权评测真实引擎与房间体验。
3. 以独立规格评测 AEC、降噪、语音驱动打断、免手连续对话与常驻恢复。
4. 实现受策略约束的会话与记忆服务。
5. 在常驻 Mac mini 上验证房间声学、恢复能力、成本和延迟。
6. 在任何现实世界副作用之前，先加入低风险、只读的家庭集成。
