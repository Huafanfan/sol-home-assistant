...
| 腾讯云实时 ASR/TTS 适配器 | 已实现，待真实 Probe | 固定官方主机签名、16k PCM 流、部分/最终转写、音频分片、超时、取消、脱敏错误和默认无网络诊断。 |
| 真实麦克风、扬声器、唤醒词、VAD | 计划中 | 在腾讯云受控 Probe 通过后，把 Mac mini 开发卫星接入真实音频设备。 |
| 长期记忆 | 计划中 | 保留策略、数据模型、删除语义和审计边界已经先行定义。 |
| 智能家居写操作、联网搜索 | 不在 MVP 范围内 | 这些能力必须单独定义权限、确认、成本和审计规则。 |

一句话概括：会话、文本推理与腾讯云语音适配器已经存在并完成自动化测试；腾讯云语音真实调用和第一轮端到端语音对话仍在后续验收中。
