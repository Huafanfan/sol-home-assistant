...
| 唤醒词与自动 VAD | 规格已接受，未实现 | VOICE-005 第一片将在前台本地监听中实现零未唤醒云调用与自动结束输入；真实模型与房间验收尚未开始。 |
...
一句话概括：文本推理与腾讯云实时语音已经完成真实验收，VOICE-004 已完成 Mac mini 的真实中文单轮、启动前零调用、手动打断及用量/无落盘复核；VOICE-005 的本地唤醒与自动 VAD 第一片已完成规格与 ADR，下一步才是离线实现。
...
2. 实现 VOICE-005 第一片：前台本地唤醒、自动 VAD、协议能力协商与离线替身。
