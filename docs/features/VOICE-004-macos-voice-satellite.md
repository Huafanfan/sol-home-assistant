...
- [ ] 首次运行能通过 macOS 系统流程获得麦克风权限；拒绝时不联网，允许后状态清晰可见。2026-08-20 用户决定暂缓真实拒绝/受限路径，不将其记为通过。
- [x] 使用系统默认麦克风手动说一句中文，完成麦克风→ASR→Gateway/文本推理→TTS→本机扬声器的一轮真实闭环；2026-08-20 复测结果为 `completed`，用户确认回答清晰可听。
- [x] 未手动开始时，安全指标和腾讯云控制台均证明没有 ASR 调用或上传音频；2026-08-20 用户确认基线零调用。
- [x] 播放回答时执行本地手动打断，扬声器及时停止，Gateway/ASR/TTS 取消，取消后无音频或文本继续交付；2026-08-20 用户确认立即静音，自动化测试覆盖取消后零交付。
- [ ] 切换或断开默认设备时当前轮安全停止；恢复设备后可由用户手动开始新一轮，不需重启整套服务。2026-08-20 用户决定暂缓真实设备变化路径，不将其记为通过。
- [x] 本轮日志、工作目录和 Git 状态证明没有生成录音、完整转写、回答或凭据文件；控制台用量符合单轮上限；2026-08-20 用户确认 `git status` 为空、无新增文件且 ASR/TTS 用量正常。
- [x] 用户完成主观验收：音量可听清、无明显爆音/严重卡顿，开始/结束/打断状态可理解；2026-08-20 用户确认回答清晰可听且手动打断立即生效，未报告明显爆音或严重卡顿。

2026-08-20 用户明确将“真实设备切换/断开”和“真实拒绝/受限权限路径”暂缓到后续实际遇到时再测试和修复。两项保留未勾选、不是通过证据；不改变本规格仍为 `implemented` 的状态。
...
| 真实运行/人工验收 | 2026-08-19：用户在真实 Mac 上授权麦克风，使用系统默认输入完成约 6–9 秒中文采集，并实际经过腾讯 ASR、Gateway、文本推理、腾讯 TTS 和本机扬声器听到完整回答。2026-08-20：用户复测确认未 `start` 时腾讯云零调用；一轮真实中文对话为 `completed` 且回答清晰可听；播放中 `interrupt` 后立即静音；`git status` 为空、无新增文件、ASR/TTS 用量正常。真实设备切换/断开与拒绝/受限权限路径由用户明确暂缓，因此规格保持 `implemented` |
| 已知限制或未验证假设 | 裸 SwiftPM 可执行文件的 TCC 权限归属及真实拒绝/受限路径、实际默认设备变化、真实播放欠载可观测性、采集/播放延迟、音量/爆音/卡顿和取消停止延迟仍需真实 Mac mini 验收；其中权限拒绝与设备变化已由用户暂缓。唤醒/VAD/AEC、常驻恢复和签名发布仍不在 VOICE-004 范围内 |
...
| 2026-08-20 | 补充真实硬件验收与暂缓决策 | 本规格的真实硬件验收、隐私、失败/取消语义、START-HERE、功能目录与公共 README | 用户确认启动前腾讯云零调用、真实中文单轮 `completed` 且清晰可听、播放中手动打断立即静音、工作区无新增文件且 ASR/TTS 用量正常；真实设备变化与拒绝/受限权限路径由用户暂缓，保留未勾选，状态保持 `implemented` |
