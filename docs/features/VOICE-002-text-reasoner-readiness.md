...
- [x] 配置校验拒绝缺失、空白、非法 URL、非 HTTPS（非环回例外）和无模型配置；错误不含密钥值、长度或指纹。
- [x] `.env` 同步只在文件不存在时创建，设置 0600，不覆盖已有文件，且不会出现在 `git status`。
- [x] 模型列表、普通 Chat Completions、SSE/流式片段、401/429/5xx、超时和 `AbortSignal` 均由本地 mock 覆盖。
- [x] 实际适配器向上游只发送允许的文本字段和固定元数据；测试证明请求 JSON 不含音频、完整会话、长期记忆、密钥或工具定义。
- [x] Probe report 不包含 endpoint、Authorization、密钥、请求正文或完整响应正文。
- [x] 适配器可被 VOICE-001 注入，用户取消会阻止后续文本片段进入 TTS。

受控真实探测（用户已授权，但不会伪称生产验证）：

- [x] 使用本机 `.env` 成功列出上游模型 ID，不输出 endpoint 或凭据。
- [x] 至少一个候选模型完成固定短文本的非流式与流式请求，并记录脱敏延迟。
- [x] 对选定模型验证取消可终止本地请求；当前观测到本地请求取消，服务端是否已停止生成仍不可断言。
- [x] `TEXT_REASONER_MODEL` 被设置为依据上述优先级选择的实测可用模型。
...
| 项目 | 证据 |
| --- | --- |
| 实现路径 | `packages/text-reasoner/src/`（安全配置、错误分类、OpenAI-compatible 客户端）、`apps/voice-gateway/src/config/text-reasoner.ts`（Gateway 组合）、`scripts/bootstrap-text-reasoner-env.ts` 与 `scripts/probe-text-reasoner.ts` |
| 文档/ADR 更新 | 本规格、`docs/START-HERE.md`、`docs/features/README.md`、`README.md` 与 `.env.example`；不需要新增 ADR |
| 静态检查 | `npm run typecheck` 通过 |
| 自动化测试 | `npm test` 通过，18/18；覆盖配置脱敏、URL 归一化、`.env` 创建/拒绝覆盖、模型列表、普通/流式 Chat Completions、401/429/5xx、超时、取消、VOICE-001 注入和 TTS 截止边界 |
| 真实运行/人工验收 | 已通过受测试的本机引导脚本创建 0600 且被 Git 忽略的 `.env`；`npm run probe:text-reasoner` 成功列出 9 个模型，确认 `gpt-5.6-terra` 已列出，固定短文本普通响应约 3.4 秒、首个可朗读片段约 1.4 秒，并观测到本地取消。未显示 endpoint、凭据或响应正文 |
| 已知限制或未验证假设 | 上游价格/配额、复杂推理质量、并发容量及服务端是否在客户端取消后停止生成仍未知；腾讯云 ASR/TTS、麦克风、唤醒词、VAD、记忆和工具仍不在本功能范围内；尚未设计自动模型路由或“深度模式”产品交互 |

## 10. 复核记录

| 日期 | session / 变更 | 阅读和复核的文档 | 结论 |
| --- | --- | --- | --- |
| 2026-08-16 | 实现与受控验收 | 本规格、ADR-0002/4、VOICE-001；实现安全配置、文本适配器、`.env` 引导、Probe CLI 与 mock 测试，并完成当前 `.env` 的实际 Probe | `verified`；`gpt-5.6-terra` 已作为本机默认文本深度推理模型 |
