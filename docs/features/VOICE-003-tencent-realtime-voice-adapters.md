...
| 已知限制或未验证假设 | Surge 精确直连已消除 ASR `6001`；真实成功路径、脱敏指标与独立本地取消边界均已验收。本地取消只能证明客户端关闭和取消后零交付，不能证明腾讯云停止生成或计费。仍需用户在腾讯云控制台核对本轮调用/用量未超规格上限，完成前保持 `implemented`；本机音频、VAD/AEC/唤醒词不在本功能范围 |
