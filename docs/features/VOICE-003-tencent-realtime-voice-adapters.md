...
- [ ] mock TTS 覆盖握手、READY、音频块、完成、空音频、提供方错误、超时和 `AbortSignal`；取消后没有音频进入播放或 Probe 回送。
...
- [ ] 使用新生成且只存在于本机 `.env` 的专用子账号密钥完成鉴权；报告不显示 AppID、SecretId、SecretKey、签名或完整 URL。
- [ ] 固定短文本通过实时 TTS 得到非空 16k PCM，记录首音频与完成延迟、字符数和音频秒数。
