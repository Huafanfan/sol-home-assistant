...
- [x] 临时 PCM 通过实时 ASR 得到至少一个最终结果，记录首部分/最终结果延迟和音频秒数；只以布尔/长度范围验证结果，不打印正文。
- [x] 独立取消测试证明本地连接关闭且取消后的提供方数据不会进入 VOICE-001；不宣称服务端已停止计费。
- [x] 本轮调用次数与用量未超过规格上限，并由用户复核控制台用量/计费状态。
...
| 真实运行/人工验收 | 2026-08-17 在新密钥安全门、离线签名自检通过后执行七次各自获得授权的受控 Probe。前三次定位并修正 TTS 鉴权配置；第四、第五次把问题最小化到 ASR `6001`；Surge 精确 `DIRECT` 后第六次打通功能链路。第七次最终 Probe 退出码 `0`：TTS 首包 344.872 ms、完成 357.470 ms、10 字符、13 块/94688 字节/2.959 秒 PCM；ASR 首部分 733.752 ms、最终 3179.978 ms、最终结果存在且长度 9；TTS/ASR 各 2 次连接、重试 0；两类本地取消均被观察到，取消后音频块、部分结果和最终结果交付均为 0。全程未输出或持久化凭据、正文、转写或音频 |
| 已知限制或未验证假设 | Surge 精确直连已消除 ASR `6001`；真实成功路径、脱敏指标、独立本地取消边界和控制台用量均已验收。本地取消只能证明客户端关闭和取消后零交付，不能证明腾讯云停止生成或计费；本机音频、VAD/AEC/唤醒词不在本功能范围，须由后续独立规格验收 |

## 10. 复核记录

| 日期 | session / 变更 | 阅读和复核的文档 | 结论 |
| --- | --- | --- | --- |
| 2026-08-17 | 控制台用量人工复核 | 用户确认第七次受控 Probe 后的腾讯云 ASR/TTS 用量、资源包扣减和计费状态正常 | 所有自动化与受控真实验收项均有证据；VOICE-003 从 `implemented` 更新为 `verified`，但不扩张到本机麦克风/扬声器范围 |
| 2026-08-17 | 第七次最终成功与取消受控 Probe | 用户明确授权执行 `--confirm-billable --confirm-cancellation`；只执行一次且未重试。安全单行报告记录 TTS/ASR 延迟、音频用量、连接次数和本地取消后交付计数，不含内容或凭据 | 退出码 `0`；真实成功路径与两类独立取消均通过，TTS/ASR 各 2 次连接、重试 0、取消后交付 0。只剩用户核对腾讯云控制台用量，完成前不标记 `verified` |
